...
int FDC()                                         /*подпр.обр.пс.опер.DC    */
 {
  if ( PRNMET == 'Y' )                            /*если псевдооп.DC помеч.,*/
   {                                              /*то:                     */
    ...
    else if                                       /*Если операция определяет*/
     (                                            /*константу BL2 (т.е. бит.*/
      strncmp (                                   /*строку размером 2 байта */
        TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND,     /*устанавливаем длину симв*/
        "BL2", 3                                  /*2 и сдвигаем адрес на 2 */
              ) == 0                              /*байта                   */
     )
     {
      T_SYM[ITSYM].DLSYM = 2;                     /*Если после этого будет  */
      T_SYM[ITSYM].PRPER = 'R';                   /*вызвана операция DS OF  */
      if ( CHADR % 2 )                            /*она выровняет счетчик   */
       {                                          /*по границе 4-х байтового*/
        CHADR = (CHADR /2 +1) * 2;                /*слова                   */
...
int SDC()                                         /*подпр.обр.пс.опер.DC    */
 {
  ...
  if
    ...
    )                                             /* то                     */
   {
    ...
    STXT(4);                                      /*Формируем карту для RX  */
   }

  else if                                         /*Если операнд начинается */
    (                                             /*с BL2, имеем дело с бит.*/
     strncmp(TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND,/*строкой размера 2 байта */
             "BL2", 3) == 0                       /*Поэтому целесообразно   */
    )                                             /*использовать операцию RR*/
   {
    RAB = strtok (                                /*Считываем бит. строку   */
      (char*)TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND /*из операнда             */
      + 4, "'" );

    char BL2_STR[16];                             /*Макс. кол-во символов в */
    strcpy ( BL2_STR, RAB );                      /*бит. строке - 16        */
    memset ( BL2_STR + strlen(RAB),
             '0', 16 - strlen(RAB) );

    char BYTE1[9]; BYTE1[8] = '\0';               /*Разделяем строку по     */
    strncpy(BYTE1, BL2_STR, 8);                   /*байтам и записываем в   */
    char BYTE2[9]; BYTE2[8] = '\0';               /*два поля OP_RR, каждое  */
    strncpy(BYTE2, BL2_STR + 8, 8);               /*по 1 байту              */
    RR.OP_RR.OP = strtol ( BYTE1, NULL, 2 );
    RR.OP_RR.R1R2 = strtol ( BYTE2, NULL, 2 );

    STXT(2);                                      /*Формируем карту для RR  */
   }
  else                                            /*иначе                   */
...
int SDS()                                         /*подпр.обр.пс.опер.DS    */
 {
  ...
  if
   ...
   RX.OP_RX.B2D2 = 0;                             /*занулим RX.OP_RX.B2D2   */
    ...
    (                                             /*Если операнд 0F, то     */
     strncmp(TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND,/*выравниваем счетчик по  */
             "0F", 2) == 0                        /*границе 4-х байтового   */
    )                                             /*слова, если это необх.  */
   {
    if ( CHADR % 4 )
      CHADR = (CHADR /4 + 1) * 4;
    return (0);                                   /*Не формируем карту      */
   }
  else
    return (1);
